...
default: $(TARGETS) frf_transform_base.so frf_callback.so libfrf.a
...
test.frf: data/content.json data/p13n.txt make_frf frf_transform_base.so frf_callback.so
...
frf_transform_base.so: frf_transform_base.o frf_malloc.o
	$(CC) $(CFLAGS) -shared $^ -o frf_transform_base.so

frf_callback.so: frf_callback.o frf_malloc.o
	$(CC) $(CFLAGS) -shared $^ -o frf_callback.so
